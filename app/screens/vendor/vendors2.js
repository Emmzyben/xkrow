...
import useGetAllVendor from '../../hooks/useGetAllVendor22';
import { useUser } from '../../backend/user'; 

const Vendors2 = ({ navigation }) => {
  ...
  return (
    <ScrollView style={{backgroundColor:'#fff'}}>
      <View style={{ padding: 20, borderBottomWidth: 1, borderBottomColor: 'rgba(102, 112, 133, 1)' ,paddingTop:35}}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={{ position: 'absolute', top: 35, left: 30,zIndex:100000 }}>
      ...
      <View style={styles.Vendors}>
        <Text style={styles.text5}>Find Professionals</Text>
        <TextInput 
          style={styles.input1} 
          placeholder='Search' 
        ...
        <View>
          {vendors.map((vendor) => (
            <TouchableOpacity
              key={vendor.user_id}
              onPress={() => navigation.navigate('VendorProfile2', { userId: vendor.user_id })}
              style={styles.floater}
            >
              <Image style={styles.img1}   source={vendor.image_url ? { uri: vendor.image_url } : require('../../../assets/logo.png')}
 />
              <View>
