...
const Landing = ({ navigation }) => {
  const { user } = useUser();
  const { profile, loading, fetchProfile } = useGetProfile();
    ...
    try {
      ...
      if (notificationResponse.total > 0) {
        const newNotifications = notificationResponse.documents.filter(notification => {
          ...
          return lastViewedNotification && notificationDate > lastViewedNotification;
        });
        setNotificationCount(newNotifications.length);
      } else {
        setNotificationCount(0); // No notifications, so reset the count
      }
    } catch (error) {
    ...
    const loadLastViewedNotification = async () => {
      try {
        ...
        if (lastViewed) {
          ...
          setLastViewedNotification(new Date()); // Default to current time if no timestamp is found
        }
      } catch (error) {
